...
-- Обновленная структура (1:N связь с артефактами)
CREATE TABLE targets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  url TEXT NOT NULL, -- ссылка на изображение-таргет
  size_cm INTEGER DEFAULT 10, -- размер стороны квадрата в см
...
CREATE TABLE artifact_media (
  ...
  display_order INTEGER DEFAULT 0, -- 0 - наивысший приоритет
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(artifact_id, media_id) -- один артефакт может быть связан с медиа только один раз
);

-- Таблица artifact_targets удалена, так как связь теперь прямая через targets.artifact_id
