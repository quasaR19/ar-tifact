 ...
 * @returns Promise с результатом проверки качества
 */
export async function checkImageQualityFromUrl(
  url: string
): Promise<ImageQualityResult> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // Загружаем файл для получения размера
      ...
      img.onload = () => {
        try {
          // Создаем временный File объект для совместимости с analyzeImage
          const tempFile = new File([blob], fileName, { type: blob.type });
          const result = analyzeImage(img, tempFile);
          resolve(result);
        } catch (error) {
          reject(error);
        }
      ...
      reject(
        error instanceof Error ? error : new Error("Ошибка анализа изображения")
      );
    }
  });
}
